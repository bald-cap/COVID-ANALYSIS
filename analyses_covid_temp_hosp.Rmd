...
Pour cette étude on devraient connaitre quel partie de la population a été le plus impacté par le covid-19. Pour cela on doit se servir des taux des soins intensifs calculés par tranche d'âge, et grâce à ces données on pourra construire un histogramme clarifiant les tranches d'âges les plus impactés.
```{r GRAPHE DE CAS INTENSIFS PAR TRANCHE D'AGE}
...
Pour Répondre à la question poser précédemment on vas utiliser les taux de morts calculer par tranches d'âges. Grâce à ce resultat on pourra savoir quel tranche d'âge qui a subit le plus de décès et en même temps de voir si le covid-19 à eu une grande influence sur le taux de mort.
...
```{r HISTOGRAMMES DÉMONTRANT LA RELATION ENTRE LES TAUX DE CAS INTENSIFS ET LES TAUX DE MORTALITÉS}
...
Maintenant qu'on a une vu plus globale dans ce qui concerne le pays par tranches d'âges, pour clarifier plus nos recherches on doit savoir si la temperatures était un facteur majeur qui influe sur les soins intensif. Pour ça en comparera le coté NORD de la france connue pour ces températures basses et coté SUD qui est le cas contraire, en créant une carte thermique pour mieux visualier les valeurs. Grâce â cette comparaison on pourra conclure sur le lien entre température et décès.

```{r CARTE THERMIQUE DES TEMPÉRATURES MOYENNES ET DES TAUX DE SOINS INTENSIFS RÉGIONAUX}
library(pheatmap)
...
temps_per_reg <- dbGetQuery(
  ...
  "SELECT
    ...
    SUM(NUM_INTENSE_CARE)/ SUM(total) * 10000 AS INTENSE_CARE_RATE
  FROM PROJET_DEP D
  INNER JOIN PROJET_REG R
  ON R.codeR = D.codeR
  INNER JOIN PROJET_POPU P
  ON P.codeD = D.codeD
  INNER JOIN TEMPERATURES T
  ON T.DEP_ID = D.codeD
  INNER JOIN HOSPITALISATIONS H
  ON H.DEP_ID = T.DEP_ID
  WHERE YEAR(T.DATE_TEMP) IN (2020)
  GROUP BY REGS"
)

data_matrix <- as.matrix(
  temps_per_reg[,
    c("AVG_TEMPS", "INTENSE_CARE_RATE")
  ]
)

rownames(data_matrix) <- temps_per_reg$REGS
...
pheatmap(data_matrix,
  ...
  main = "Heatmap of Average Temperatures and Intensive Care Rates by Region",
  annotation_row = data.frame(Region = temps_per_reg$REGS)
)

```
Sur la base des données représentées sur la carte thermique, on peut conclure que la région Sud de la France présente une température moyenne plus élevée (14.19°C) par rapport à la région Nord (12.90°C). Cependant, malgré des températures plus clémentes, le taux de soins intensifs dans la région Sud est inférieur (0.2213 pour 10 000 habitants) à celui de la région Nord (0.2580 pour 10 000 habitants). 

### LA CORRELATION ENTRE LE TAUX DE CAS INTENSE ET LE TAUX DE MORTALITE SUR TOUTE LA POPULATION
On a pu conclure la correlation entre les taux de cas intenses et les taux de mortalites par tranches d'âges, mais sa sera plus intéressant de voir coté population qu'est ce ça donne. ça aidera mieux visualiser les changements démographique à une échelle plus grande.
...
Nous allons maintenant réaliser un test de Student pour vérifier si nous pouvons rejeter l'hypothèse nulle concernant le coefficient de corrélation tout en confirmant ce que les conclusions statistiques sont bien refletent les conclusions deduites.
```{r T TEST}
...
Pour visualiser mieux l'activiter saisonial des hopitaux on aimerais étudier le taux d'hospitalisations en fonctions de saisons.
