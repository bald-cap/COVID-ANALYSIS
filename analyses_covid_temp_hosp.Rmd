...
Dans le cadre de notre investigation, nous nous penchons sur une analyse approfondie des données d'hospitalisation au cours de la période marquée par la pandémie de COVID-19, s'étalant sur les années 2019 et 2020, tout en intégrant 2018 et 2021 comme jalons temporels pour une perspective élargie pré et post-pandémique. L'objectif initial est d'analyser les taux d'hospitalisation pour en dégager des tendances et identifier les moments clés ou les fluctuations notables. Par la suite, notre attention se tourne vers les impacts potentiels des variations de température sur ces taux d'hospitalisation, cherchant à décrypter l'influence des conditions climatiques sur la dynamique de la santé publique durant cette crise sanitaire. En culmination, nous orchestrons une analyse bivariée, conjuguant températures et données hospitalières, pour explorer l'existence d'une corrélation entre ces deux variables. Cette démarche méthodologique repose sur l'application de techniques statistiques avancées, telles que le coefficient de corrélation de Pearson et l'analyse de séries temporelles, appuyées par des représentations graphiques, afin de synthétiser et d'interpréter nos découvertes avec précision. Notre étude aspire à révéler des insights significatifs sur l'interaction entre l'environnement et la santé au cours d'une période critique, enrichissant ainsi notre compréhension des facteurs influençant les taux d'hospitalisation en contexte pandémique.
```{r access a la base de données}
library(RMariaDB)
