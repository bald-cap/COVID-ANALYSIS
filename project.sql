...
SELECT 
    ...
    SUM(H.NUM_INTENSE_CARE) * age0_19/total AS Intensive_Care_0_19,
    SUM(H.NUM_INTENSE_CARE) * age20_39/total AS Intensive_Care_20_39,
    SUM(H.NUM_INTENSE_CARE) * age40_59/total AS Intensive_Care_40_59,
    SUM(H.NUM_INTENSE_CARE) * age60_74/total AS Intensive_Care_60_74,
    SUM(H.NUM_INTENSE_CARE) * age75_et_plus/total AS Intensive_Care_75_Plus
FROM HOSPITALISATIONS H
INNER JOIN PROJET_DEP PD 
    ON H.DEP_ID = PD.codeD
INNER JOIN PROJET_POPU PP 
    ON PD.codeD = PP.codeD
INNER JOIN PROJET_REG PR 
    ON PD.codeR = PR.codeR
WHERE YEAR(H.DATE_HOSP) IN (2020, 2021)
GROUP BY YEAR(H.DATE_HOSP)
ORDER BY YEAR(H.DATE_HOSP);

-- POPULATION AND DEATHS
SELECT 
    YEAR(H.DATE_HOSP) AS Year,
    SUM(H.NUM_DEATHS) * age0_19/total AS DEATHS_0_19,
    SUM(H.NUM_DEATHS) * age20_39/total AS DEATHS_20_39,
    SUM(H.NUM_DEATHS) * age40_59/total AS DEATHS_40_59,
    SUM(H.NUM_DEATHS) * age60_74/total AS DEATHS_60_74,
    SUM(H.NUM_DEATHS) * age75_et_plus/total AS DEATHS_75_Plus
FROM HOSPITALISATIONS H
INNER JOIN PROJET_DEP PD 
    ON H.DEP_ID = PD.codeD
INNER JOIN PROJET_POPU PP 
